...
describe('#reverseByRange', function() {
  ...
  it('should iterate correctly by duration', function() {
    ...
    const dr1 = DateRangeCreate(d1, d2);
    const dr2 = 1000*15*60;//duration(15, 'minutes');  number has method valueOf

    const acc = Array.from(dr1.reverseByRange(dr2));

    expect(acc.length).to.eql(96);
    expect(acc[0].getMinutes()).to.eql(0);
    expect(acc[95].getMinutes()).to.eql(15);
  });
  ...
  it('should not include .start in the iteration if exclusive is set to true when iterating by range', function() {
    ...
    acc = Array.from(dr1.reverseByRange(dr2)).map(m => m.getFullYear()+"-"+(m.getMonth()+1)+"-"+m.getDate());
    expect(acc).to.eql(['2014-4-4', '2014-4-3', '2014-4-2']);

    acc = Array.from(dr1.reverseByRange(dr2, { exclusive: false })).map(m => m.getFullYear()+"-"+(m.getMonth()+1)+"-"+m.getDate());
    expect(acc).to.eql(['2014-4-4', '2014-4-3', '2014-4-2']);

    acc = Array.from(dr1.reverseByRange(dr2, { exclusive: true })).map(m => m.getFullYear()+"-"+(m.getMonth()+1)+"-"+m.getDate());
  ...
  it('should iterate correctly by a given step', function() {
    ...
    const acc = Array.from(dr1.reverseByRange(dr2, { step: 2 })).map(m => m.getDate());

    expect(acc).to.eql(['6', '4', '2']);
  });
  ...
  it('should iterate correctly by a given step when exclusive', function() {
    ...
    const acc = Array.from(dr1.reverseByRange(dr2, { exclusive: true, step: 2 })).map(m => m.getDate());

    expect(acc).to.eql(['6', '4']);
  });
});
